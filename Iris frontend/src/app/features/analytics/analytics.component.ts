  ...
  template: `
    <div class="page">
      <div class="page-header">
        <div><h1>Data Analytics & Reporting</h1><p>Advanced analytics, visualization & reports</p></div>
      ...
      <div class="hero-section"><img src="assets/images/data-analytics-bg.png" class="hero-img" /><div class="hero-ov"><h2>Data Analytics & Reporting</h2><p>Advanced analytics, visualization & reports</p></div></div>
      <div class="kpi-row">
        <div class="kpi-card"><div class="kpi-icon blue"><i class="pi pi-chart-bar"></i></div><div><span class="kpi-val">12</span><span class="kpi-lbl">Dashboards</span></div></div>
        <div class="kpi-card"><div class="kpi-icon green"><i class="pi pi-file"></i></div><div><span class="kpi-val">156</span><span class="kpi-lbl">Reports</span></div></div>
        <div class="kpi-card"><div class="kpi-icon orange"><i class="pi pi-clock"></i></div><div><span class="kpi-val">24</span><span class="kpi-lbl">Scheduled</span></div></div>
        <div class="kpi-card"><div class="kpi-icon red"><i class="pi pi-database"></i></div><div><span class="kpi-val">8</span><span class="kpi-lbl">Sources</span></div></div>
      </div>
      <div class="tabs">
        <button class="tab" [class.active]="activeTab() === 'assets'" (click)="activeTab.set('assets')">Data Sources</button>
      ...
      @if (activeTab() === 'reports') {
        <div class="data-table"><table><thead><tr><th>Report</th><th>Category</th><th>Format</th><th>Last Run</th><th>Schedule</th><th>Actions</th></tr></thead><tbody><tr><td><strong>Monthly Risk Report</strong></td><td>Risk</td><td>PDF</td><td>Feb 1</td><td>Monthly</td><td class="actions-cell"><button class="icon-btn" (click)="interact.success('Downloading...')"><i class="pi pi-download"></i></button><button class="icon-btn" (click)="interact.info('Scheduling...')"><i class="pi pi-clock"></i></button></td></tr><tr><td><strong>Weekly Fraud Summary</strong></td><td>Fraud</td><td>Excel</td><td>Feb 14</td><td>Weekly</td><td class="actions-cell"><button class="icon-btn" (click)="interact.success('Downloading...')"><i class="pi pi-download"></i></button><button class="icon-btn" (click)="interact.info('Scheduling...')"><i class="pi pi-clock"></i></button></td></tr><tr><td><strong>Quarterly Compliance</strong></td><td>Compliance</td><td>PDF</td><td>Jan 5</td><td>Quarterly</td><td class="actions-cell"><button class="icon-btn" (click)="interact.success('Downloading...')"><i class="pi pi-download"></i></button><button class="icon-btn" (click)="interact.info('Scheduling...')"><i class="pi pi-clock"></i></button></td></tr><tr><td><strong>Daily Txn Monitor</strong></td><td>Transaction</td><td>CSV</td><td>Today</td><td>Daily</td><td class="actions-cell"><button class="icon-btn" (click)="interact.success('Downloading...')"><i class="pi pi-download"></i></button><button class="icon-btn" (click)="interact.info('Scheduling...')"><i class="pi pi-clock"></i></button></td></tr></tbody></table></div>
      }

      
      @if(activeTab()==='scheduled'){
        <div style="display:flex;justify-content:space-between;margin-bottom:12px"><h3 style="font-size:16px;font-weight:700;margin:0">Scheduled Reports</h3><button class="btn-primary btn-sm" (click)="interact.info('Creating schedule...')"><i class="pi pi-plus"></i> New Schedule</button></div>
        <div class="data-table"><table><thead><tr><th>Report</th><th>Frequency</th><th>Recipients</th><th>Format</th><th>Next Run</th><th>Status</th><th>Actions</th></tr></thead><tbody>
          <tr><td><strong>Daily Fraud Summary</strong></td><td>Daily 7AM</td><td>fraud-team&#64;iris.com</td><td>PDF+Excel</td><td>Feb 18, 7:00</td><td><span class="status-pill s-active">ACTIVE</span></td><td class="actions-cell"><button class="icon-btn" (click)="interact.info('Editing...')"><i class="pi pi-pencil"></i></button><button class="icon-btn" (click)="interact.success('Running now...')"><i class="pi pi-play"></i></button></td></tr>
          <tr><td><strong>Weekly Risk Report</strong></td><td>Mon 8AM</td><td>cro&#64;iris.com, board&#64;iris.com</td><td>PDF</td><td>Feb 24, 8:00</td><td><span class="status-pill s-active">ACTIVE</span></td><td class="actions-cell"><button class="icon-btn" (click)="interact.info('Editing...')"><i class="pi pi-pencil"></i></button><button class="icon-btn" (click)="interact.success('Running now...')"><i class="pi pi-play"></i></button></td></tr>
          <tr><td><strong>Monthly AML Report</strong></td><td>1st of month</td><td>compliance&#64;iris.com</td><td>PDF</td><td>Mar 01, 6:00</td><td><span class="status-pill s-active">ACTIVE</span></td><td class="actions-cell"><button class="icon-btn" (click)="interact.info('Editing...')"><i class="pi pi-pencil"></i></button><button class="icon-btn" (click)="interact.success('Running now...')"><i class="pi pi-play"></i></button></td></tr>
          <tr><td><strong>Quarterly Board Report</strong></td><td>Quarterly</td><td>board&#64;iris.com</td><td>PPTX</td><td>Apr 01, 9:00</td><td><span class="status-pill s-draft">PAUSED</span></td><td class="actions-cell"><button class="icon-btn" (click)="interact.info('Editing...')"><i class="pi pi-pencil"></i></button><button class="icon-btn" (click)="interact.success('Resumed')"><i class="pi pi-play"></i></button></td></tr>
        </tbody></table></div>
      }

      @if(activeTab()==='builder'){
        <div style="text-align:center;padding:40px">
          <div style="width:80px;height:80px;border-radius:20px;background:linear-gradient(135deg,#eff6ff,#dbeafe);display:flex;align-items:center;justify-content:center;margin:0 auto 16px"><i class="pi pi-th-large" style="font-size:32px;color:#3b82f6"></i></div>
          <h3 style="font-size:18px;font-weight:700;margin:0 0 8px">Dashboard Builder</h3>
          <p style="font-size:14px;color:#64748b;margin:0 0 20px;max-width:500px;margin-left:auto;margin-right:auto">Drag and drop widgets to create custom dashboards. Choose from charts, KPIs, tables, and maps.</p>
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;max-width:600px;margin:0 auto 24px">
            <div style="padding:16px;border:2px dashed #e2e8f0;border-radius:12px;cursor:pointer;text-align:center" (click)="interact.info('Adding Line Chart widget')"><i class="pi pi-chart-line" style="font-size:24px;color:#3b82f6;display:block;margin-bottom:4px"></i><span style="font-size:11px;color:#64748b">Line Chart</span></div>
            <div style="padding:16px;border:2px dashed #e2e8f0;border-radius:12px;cursor:pointer;text-align:center" (click)="interact.info('Adding Bar Chart widget')"><i class="pi pi-chart-bar" style="font-size:24px;color:#8b5cf6;display:block;margin-bottom:4px"></i><span style="font-size:11px;color:#64748b">Bar Chart</span></div>
            <div style="padding:16px;border:2px dashed #e2e8f0;border-radius:12px;cursor:pointer;text-align:center" (click)="interact.info('Adding KPI widget')"><i class="pi pi-hashtag" style="font-size:24px;color:#22c55e;display:block;margin-bottom:4px"></i><span style="font-size:11px;color:#64748b">KPI Card</span></div>
            <div style="padding:16px;border:2px dashed #e2e8f0;border-radius:12px;cursor:pointer;text-align:center" (click)="interact.info('Adding Table widget')"><i class="pi pi-table" style="font-size:24px;color:#f59e0b;display:block;margin-bottom:4px"></i><span style="font-size:11px;color:#64748b">Data Table</span></div>
          </div>
          <div style="display:grid;grid-template-columns:2fr 1fr;gap:12px;max-width:600px;margin:0 auto;height:200px">
            <div style="border:2px dashed #cbd5e1;border-radius:12px;display:flex;align-items:center;justify-content:center;background:#f8fafc"><span style="font-size:14px;color:#94a3b8"><i class="pi pi-plus-circle" style="margin-right:4px"></i> Drop widgets here</span></div>
            <div style="border:2px dashed #cbd5e1;border-radius:12px;display:flex;align-items:center;justify-content:center;background:#f8fafc"><span style="font-size:14px;color:#94a3b8"><i class="pi pi-plus-circle" style="margin-right:4px"></i> Drop here</span></div>
          </div>
        </div>
      }
      ...
      @if (showDetail()) {
        <div class="overlay" (click)="showDetail.set(false)">
          <div class="side-panel" (click)="$event.stopPropagation()">
            ...
            <div class="panel-tabs"><button class="panel-tab" [class.active]="detailTab()==='info'" (click)="detailTab.set('info')">Info</button><button class="panel-tab" [class.active]="detailTab()==='config'" (click)="detailTab.set('config')">Configuration</button><button class="panel-tab" [class.active]="detailTab()==='preview'" (click)="detailTab.set('preview')">Preview</button></div>
            <div class="panel-body">
              ...
              @if (editMode()) {
                ...
                <div class="field"><label>Type</label><select [(ngModel)]="newItem.type"><option value="">Select...</option><option value="TABLE">TABLE</option><option value="VIEW">VIEW</option><option value="API">API</option><option value="FILE">FILE</option></select></div>
...
export class AnalyticsComponent implements OnInit {
  ...
  applyFilters() {
    let r = [...this.allData];
    if (this.searchTerm) { const q = this.searchTerm.toLowerCase(); r = r.filter(item => ['id','name','source'].some((f: string) => String((item as any)[f] || '').toLowerCase().includes(q))); }
  ...
  createItem() {
    const id = 'ANA-' + String(Date.now()).slice(-6);
  ...
  exportData() {
    ...
    this.interact.exportCSV(this.filtered, 'analytics', cols);
  }
}
