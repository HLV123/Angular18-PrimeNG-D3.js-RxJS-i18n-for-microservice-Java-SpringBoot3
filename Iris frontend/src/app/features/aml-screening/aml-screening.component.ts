  ...
  template: `
    <div class="page">
      <div class="page-header">
        <div><h1>AML & Sanctions Screening</h1><p>Anti-money laundering, customer risk & sanctions screening</p></div>
      ...
      <div class="hero-section"><img src="assets/images/aml-screening-bg.png" class="hero-img" /><div class="hero-ov"><h2>AML & Sanctions Screening</h2><p>Anti-money laundering, customer risk & sanctions screening</p></div></div>
      <div class="kpi-row">
        <div class="kpi-card"><div class="kpi-icon blue"><i class="pi pi-bolt"></i></div><div><span class="kpi-val">18,432</span><span class="kpi-lbl">Screened Today</span></div></div>
        <div class="kpi-card"><div class="kpi-icon red"><i class="pi pi-bell"></i></div><div><span class="kpi-val">42</span><span class="kpi-lbl">AML Alerts</span></div></div>
        <div class="kpi-card"><div class="kpi-icon orange"><i class="pi pi-users"></i></div><div><span class="kpi-val">156</span><span class="kpi-lbl">High Risk</span></div></div>
        <div class="kpi-card"><div class="kpi-icon green"><i class="pi pi-file"></i></div><div><span class="kpi-val">8</span><span class="kpi-lbl">SARs Filed</span></div></div>
      </div>
      <div class="tabs">
        <button class="tab" [class.active]="activeTab() === 'alerts'" (click)="activeTab.set('alerts')">AML Alerts</button>
        <button class="tab" [class.active]="activeTab() === 'customers'" (click)="activeTab.set('customers')">Customers</button>
        <button class="tab" [class.active]="activeTab() === 'sanctions'" (click)="activeTab.set('sanctions')">Sanctions</button>
      </div>
      ...
      @if (activeTab() === 'alerts') {
        ...
        <div class="data-table">
          <table>
            <thead><tr><th (click)="sortBy('id')">ID</th><th (click)="sortBy('customerName')">Customer</th><th (click)="sortBy('scenario')">Scenario</th><th (click)="sortBy('riskScore')">Risk Score</th><th (click)="sortBy('priority')">Priority</th><th (click)="sortBy('status')">Status</th><th (click)="sortBy('totalAmount')">Amount</th><th>Actions</th></tr></thead>
            <tbody>
              @for (item of paginated(); track $index) {
                <tr (click)="viewItem(item)" [class.selected]="selectedItem?.id === item.id">
                  <td>{{ item.id }}</td><td>{{ item.customerName }}</td><td>{{ item.scenario }}</td><td class="mono">{{ item.riskScore | number }}</td><td><span class="priority-tag" [class]="'p-' + (item.priority || '').toLowerCase()">{{ item.priority }}</span></td><td><span class="status-pill">{{ item.status }}</span></td><td class="mono">{{ item.totalAmount | number }}</td>
      ...
      @if (activeTab() === 'customers') {
        <div class="filter-bar"><input type="text" placeholder="Search customers..." class="search-input" [(ngModel)]="custSearch" (input)="filterCust()" /><select [(ngModel)]="custFilter" (change)="filterCust()" class="filter-select"><option value="">All Risk</option><option>HIGH</option><option>MEDIUM</option><option>LOW</option></select><span class="result-count">{{ filteredCust.length }} customers</span></div>
        <div class="data-table"><table><thead><tr><th>ID</th><th>Name</th><th>Type</th><th>Risk Score</th><th>Risk Level</th><th>PEP</th><th>KYC</th><th>Country</th><th>Actions</th></tr></thead><tbody>@for (c of filteredCust.slice(0,12); track $index) {<tr (click)="viewCust(c)"><td class="mono">{{ c.id }}</td><td><strong>{{ c.name }}</strong></td><td>{{ c.type }}</td><td class="mono">{{ c.riskScore }}</td><td><span class="priority-tag" [class]="'p-' + (c.riskCategory||'').toLowerCase()">{{ c.riskCategory }}</span></td><td>@if(c.isPEP){<span style="color:#ef4444;font-weight:700">PEP</span>}@else{-}</td><td><span class="status-pill">{{ c.kycStatus }}</span></td><td>{{ c.country }}</td><td class="actions-cell"><button class="icon-btn" (click)="viewCust(c);$event.stopPropagation()"><i class="pi pi-eye"></i></button></td></tr>}</tbody></table></div>
      }

      @if (activeTab() === 'sanctions') {
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:16px;"><div class="kpi-card"><div class="kpi-icon blue"><i class="pi pi-globe"></i></div><div><span class="kpi-val">OFAC</span><span class="kpi-lbl">12,456 entries</span></div></div><div class="kpi-card"><div class="kpi-icon green"><i class="pi pi-globe"></i></div><div><span class="kpi-val">EU</span><span class="kpi-lbl">8,234 entries</span></div></div><div class="kpi-card"><div class="kpi-icon orange"><i class="pi pi-globe"></i></div><div><span class="kpi-val">UN</span><span class="kpi-lbl">5,678 entries</span></div></div></div>
        <div class="data-table"><table><thead><tr><th>ID</th><th>Customer</th><th>List</th><th>Score</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead><tbody><tr><td class="mono">SCR-001</td><td>Công ty Alpha</td><td>OFAC SDN</td><td><span style="color:#ef4444;font-weight:700">78%</span></td><td><span class="status-pill" style="background:#fef2f2;color:#ef4444">MATCH</span></td><td>Feb 15</td><td class="actions-cell"><button class="icon-btn" (click)="interact.warn('Reviewing match SCR-001')"><i class="pi pi-eye"></i></button></td></tr><tr><td class="mono">SCR-002</td><td>Trần Holdings</td><td>EU List</td><td><span style="color:#f59e0b;font-weight:700">45%</span></td><td><span class="status-pill">REVIEW</span></td><td>Feb 15</td><td class="actions-cell"><button class="icon-btn" (click)="interact.info('Reviewing SCR-002')"><i class="pi pi-eye"></i></button></td></tr><tr><td class="mono">SCR-003</td><td>Batch 1,245</td><td>ALL</td><td><span style="color:#22c55e;font-weight:700">0%</span></td><td><span class="status-pill" style="background:#f0fdf4;color:#22c55e">CLEAR</span></td><td>Feb 15</td><td class="actions-cell"><button class="icon-btn" (click)="interact.info('Viewing batch results')"><i class="pi pi-eye"></i></button></td></tr></tbody></table></div>
      }
          ...
          <div class="side-panel" (click)="$event.stopPropagation()">
            ...
            <div class="panel-body">
              <div class="panel-tabs">
                ...
                <button class="panel-tab" [class.active]="detailTab()==='customer'" (click)="detailTab.set('customer')">Customer Profile</button>
                <button class="panel-tab" [class.active]="detailTab()==='sanctions'" (click)="detailTab.set('sanctions')">Screening Results</button>
                <button class="panel-tab" [class.active]="detailTab()==='txns'" (click)="detailTab.set('txns')">Transactions</button>
                <button class="panel-tab" [class.active]="detailTab()==='sar'" (click)="detailTab.set('sar')">SAR Filing</button>
              </div>
              @if (!editMode()) {
                <div class="detail-grid">
                  <div class="detail-field"><label>Alert ID</label><span>{{ selectedItem?.id }}</span></div>
                  <div class="detail-field"><label>Customer</label><span>{{ selectedItem?.customerName }}</span></div>
                  <div class="detail-field"><label>Scenario</label><span>{{ selectedItem?.scenario }}</span></div>
                  <div class="detail-field"><label>Risk Score</label><span>{{ selectedItem?.riskScore }}</span></div>
                  ...
                  <div class="detail-field"><label>Amount</label><span>{{ selectedItem?.totalAmount }}</span></div>
                </div>
              ...
              @if (editMode()) {
                ...
                <div class="field"><label>Scenario</label><input type="text" [(ngModel)]="newItem.scenario" placeholder="Enter scenario" /></div>
                <div class="field"><label>Risk Score</label><input type="number" [(ngModel)]="newItem.riskScore" placeholder="Enter risk score" /></div>
                ...
                <div class="field"><label>Amount</label><input type="number" [(ngModel)]="newItem.totalAmount" placeholder="Enter amount" /></div>
                </div>
...
export class AmlScreeningComponent implements OnInit {
  ...
  newItem: any = { customerName: '', scenario: '', riskScore: '', priority: '', totalAmount: '' };
  ...
  ngOnInit() { this.allData = this.mockData.getAMLAlerts(); this.filtered = [...this.allData]; this.customers = this.mockData.getCustomerRisks(); this.filteredCust = [...this.customers]; }
  ...
  applyFilters() {
    let r = [...this.allData];
    if (this.searchTerm) { const q = this.searchTerm.toLowerCase(); r = r.filter(item => ['id','customerName','scenario'].some((f: string) => String((item as any)[f] || '').toLowerCase().includes(q))); }
  ...
  createItem() {
    const id = 'AML-' + String(Date.now()).slice(-6);
    this.allData.unshift({ ...this.newItem, id, createdAt: new Date(), status: 'NEW' });
    this.applyFilters();
    this.newItem = { customerName: '', scenario: '', riskScore: '', priority: '', totalAmount: '' };
    this.showCreate.set(false);
    this.interact.success('Created: ' + id);
  }

  viewCust(c: any) { this.selectedItem = c; this.editMode.set(false); this.detailTab.set('info'); this.showDetail.set(true); }
  filterCust() {
    let r = [...this.customers];
    if (this.custSearch) { const q = this.custSearch.toLowerCase(); r = r.filter(c => c.name.toLowerCase().includes(q) || c.id.toLowerCase().includes(q)); }
    if (this.custFilter) r = r.filter(c => c.riskCategory === this.custFilter);
    this.filteredCust = r;
  }
  exportData() {
    const cols = ['id', 'customerName', 'scenario', 'riskScore', 'priority', 'status', 'totalAmount'];
    this.interact.exportCSV(this.filtered, 'aml-screening', cols);
  }
}
